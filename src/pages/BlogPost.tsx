      ...
      <article className="container mx-auto px-4 py-12 max-w-4xl">
            ...
            {post.categories && post.categories.length > 0 && (
              <div className="ml-auto">
                {post.categories.map((category, index) => (
                  <span 
                    key={index} 
                    className="bg-primary-light text-primary text-xs px-3 py-1 rounded-full ml-2"
                  >
                    {category}
                  </span>
                ))}
        ...
        <div className="mt-12 pt-8 border-t border-gray-200">
          ...
          <div className="flex space-x-4">
            <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(window.location.href)}`} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-600">
            ...
            <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(window.location.href)}`} target="_blank" rel="noopener noreferrer" className="text-blue-800 hover:text-blue-900">
